...
const OrderMessage = () => {
    ...
    const HandleOpen = async (BuyerId)=>{
        // console.log(BuyerId)
        // const openCustomer = customers.filter((id)=> id.buyerId === BuyerId)
        // console.log(openCustomer)
        // setOpenCustomer(openCustomer)
        try{
            const response = await axios(`http://localhost:5000/currentUserData?userId=${BuyerId}`)
            if(response.status === 200){
    ...
    return (
                ...
                {fetched ? (
                    ...
                    customers.map((name, idx) => (
                        <li
                            ...
                            onClick={() => HandleOpen(name.buyerId)}
                        >
                            <img 
        ...
        <div className="w-full lg:w-2/4 p-4 flex flex-col bg-white">
            ...
            <div className="flex items-center">
                ...
                <div className="ml-4">
                <h2 className="text-sm font-semibold">Alice Johnson</h2>
            ...
            <div className="mb-4 flex">
                ...
                <div className="ml-2 bg-gray-200 p-2 rounded-lg">
                <p className="text-sm">Hey! How are you?</p>
                </div>
            </div>

            <div className="mb-4 flex justify-end">
                <div className="ml-2 bg-blue-500 text-white p-2 rounded-lg">
                <p className="text-sm">I am fine, and you?</p>
                </div>
                <img src="https://via.placeholder.com/40" alt="User" className="w-8 h-8 rounded-full" />
            </div>
